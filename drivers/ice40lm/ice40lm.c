 ...
 */
static int fw_data_send(unsigned char *data)
{
	unsigned int i,j;
...
static int fw_update_start(unsigned char *data)
{
	int retry_count = 0;
			...
			struct ice4_fpga_data *data)
{
	...
	pdata->creset_b = of_get_named_gpio_flags(np_ice40lm, "ice40lm,creset_b",
				0, &pdata->creset_b_flag);
	pdata->cdone = of_get_named_gpio_flags(np_ice40lm, "ice40lm,cdone",
				0, &pdata->cdone_flag);
	pdata->irda_irq = of_get_named_gpio_flags(np_ice40lm, "ice40lm,irda_irq",
				0, &pdata->irda_irq_flag);
	pdata->spi_si_sda = of_get_named_gpio_flags(np_ice40lm, "ice40lm,spi_si_sda",
				0, &pdata->spi_si_sda_flag);
	pdata->spi_clk_scl = of_get_named_gpio_flags(np_ice40lm, "ice40lm,spi_clk_scl",
				0, &pdata->spi_clk_scl_flag);
	pdata->spi_en_rstn = of_get_named_gpio_flags(np_ice40lm, "ice40lm,spi_en_rstn",
				0, &pdata->spi_en_rstn_flag);
	...
	if (!strncasecmp(temp_str, "LDO", 3)) {
		...
		power_type = PWR_LDO;
		pdata->ir_en = of_get_named_gpio_flags(np_ice40lm, "ice40lm,ir_en",
							0, &pdata->ir_en_flag);
...
static void ice40lm_gpio_setting(void)
{
	if (0 > gpio_request_one(g_pdata->spi_en_rstn, GPIOF_OUT_INIT_LOW, "FPGA_SPI_EN")) {
		pr_info("%s rstn is busy, free and request again\n", __func__);
		gpio_free(g_pdata->spi_en_rstn);
		gpio_request_one(g_pdata->spi_en_rstn, GPIOF_OUT_INIT_LOW, "FPGA_SPI_EN");
	}

	gpio_request_one(g_pdata->creset_b, GPIOF_OUT_INIT_HIGH, "FPGA_CRESET_B");
	gpio_request_one(g_pdata->cdone, GPIOF_IN, "FPGA_CDONE");
	gpio_request_one(g_pdata->irda_irq, GPIOF_IN, "FPGA_IRDA_IRQ");
	gpio_request_one(g_pdata->spi_en_rstn, GPIOF_OUT_INIT_LOW, "FPGA_SPI_EN");

	if (power_type == PWR_LDO)
		gpio_request_one(g_pdata->ir_en, GPIOF_OUT_INIT_LOW, "FPGA_IR_EN");

/* i2c-gpio drv already request below pins */
	gpio_free(g_pdata->spi_si_sda);
	gpio_free(g_pdata->spi_clk_scl);
	gpio_request_one(g_pdata->spi_si_sda, GPIOF_OUT_INIT_LOW, "FPGA_SPI_SI_SDA");
	gpio_request_one(g_pdata->spi_clk_scl, GPIOF_OUT_INIT_LOW, "FPGA_SPI_CLK_SCL");
}
